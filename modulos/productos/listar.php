...
$query_conteo = "SELECT COUNT(*) as total_registros FROM productos p WHERE 1=1" . $condicion_estado;

if (!empty($busqueda)) {
    $query_conteo .= " AND (p.nombre LIKE ? OR p.marca LIKE ?)";
}
...
$query = "SELECT p.*, u.nombre as eliminado_por_nombre FROM productos p LEFT JOIN usuarios u ON p.eliminado_por = u.id WHERE 1=1" . $condicion_estado;
...
$query .= " ORDER BY p.nombre ASC LIMIT $por_pagina OFFSET $offset";
        ...
        <div class="d-flex gap-2">
            ...
            <?php else: ?>
                <a href="listar.php?filtro_estado=inactivo" class="btn btn-amatista-secondary rounded-pill px-3 shadow-sm d-flex align-items-center">
                    <i class="bi bi-box-seam me-2"></i> Ver Desactivados
                </a>
            <?php endif; ?>
                                ...
                                <div class="btn-group shadow-sm rounded-3 overflow-hidden">
                                    ...
                                    <?php if($p['estado'] == 0): ?>
                                        <form method="POST" action="reactivar.php" class="d-inline">
                                            ...
                                            <button type="submit" class="btn btn-sm btn-white bg-white" title="Reactivar Producto">
                                    ...
                                    <?php else: ?>
                                        <form method="POST" action="eliminar.php" class="d-inline" onsubmit="var m=prompt('Motivo de la desactivación (obligatorio):'); if(!m || !m.trim()){ alert('Debe indicar el motivo.'); return false; } this.motivo.value=m.trim();">
                                            ...
                                            <button type="submit" class="btn btn-sm btn-white bg-white" title="Desactivar Producto">
